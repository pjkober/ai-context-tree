...
**Description:**
1. Move the check for `file-templates/` to the very beginning of `create_minimal_structure.sh` and `create_minimal_structure.ps1` scripts, showing a helpful warning/error message.
2. Add a post-setup cleanup prompt wizard at the end of both scripts to offer moving, deleting, or keeping the generator script and `file-templates/` directory.
3. Create `docs/06-initialization-guide.md` to explain how to initialize structures, copy scripts along with `file-templates/`, run them, and use the automated cleanup.
...
1. **Script Check Relocation:**
   * In `create_minimal_structure.sh`, moved directory validation of `file-templates/` right after `TEMPLATES_DIR` variable initialization (lines 11-18).
   ...
   * If `file-templates/` is missing, the script terminates immediately with a descriptive warning telling the user to copy both the script and the `file-templates/` directory.
