...
- **Core Rules:**
  - **Nazewnictwo z opisem:** Files must be sequentially numbered and descriptively named (e.g. `conver-001-setup-payments-api.md`).
  - **Zasada czystości (No Secrets Rule):** Absolutely zero hardcoded API keys, passwords, or secrets.
...
- **Core Rules:**
  - **Nazewnictwo z opisem:** Files must be sequentially numbered and descriptively named (e.g. `run-001-seed-test-users.sh`).
  - **Zasada czystości (No Secrets Rule):** Absolutely zero hardcoded API keys, passwords, or secrets. Must load environment variables from `.env`.
  - **Tylko przydatne skrypty:** Only store reusable scripts with automation value. Do not save logs of one-off throwaway commands.
