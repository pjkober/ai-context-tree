...
2. **Locate the defect:** Identify the failing code. Check `.agents/context/modules.md` to understand which module is responsible.
...
5. **Apply the minimal fix:** Fix the root cause in `src/`. Follow `.agents/rules/coding.md`. Apply only the minimal change required -- do not add new features or unrelated refactoring.
