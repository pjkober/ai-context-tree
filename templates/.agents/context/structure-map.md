...
| `.agents/skills/` | created at project start to store project-specific local agent skills |
| `.agents/history/` | created at project start to store session transcripts and conversation summaries |
| `.agents/lessons/` | created at project start to store post-mortems, lessons learned, and known issues |
| `.agents/runs/` | created at project start to store reusable automated scripts generated by agents |
| `tasks/` | when tasks start being managed via individual markdown files with YAML frontmatter |
| `tmp/` | created at project start for temporary files, local execution logs, and cache |
